...
Description
    Solver for catalyst simulation and catalytic combustion simulation
    with detailed transport, detailed chemical reactions, and detailed 
    microkinetics for surface reactions. 
    The computational time for gas-phase calculations can be reduced 
    by applying coTHERM method as the same as in DTLreactingFoam.
...
int main(int argc, char *argv[])
{
    ...
    while (pimple.run(runTime))
    {
        ...
        {
            #include "UEqn.H" 
            #include "calculateSurfaceReactions.H"           
            if (usingDetailedTransportModel)
            {
                // if using detailed transport
                // same as DTLreactingFoam
                if (soretEffect)
                {
                    #include "DTL_YEqn_soret.H"
                    #include "DTL_EEqn.H" 
                }
                else
                {
        ...
        if (usingDetailedTransportModel)
        {
            forAll(Dimix, i)
            {
                ...
                hei[i] = thermo.hei(i);             
                DimixT[i] = thermo.DimixT(i); // Soret             
            }
        }
        // ---
        runTime.write();
