...
const char* Foam::surfChemkinReader::reactionTypeNames[4] =
{
    "irreversible",
    "reversible",
...
const char* Foam::surfChemkinReader::reactionRateTypeNames[8] =
{
    "Arrhenius",
    "thirdBodyArrhenius",
    "unimolecularFallOff",
...
const char* Foam::surfChemkinReader::fallOffFunctionNames[4] =
{
    "Lindemann",
    "Troe",
    "SRI",
...
void Foam::surfChemkinReader::initReactionKeywordTable()
{
...
Foam::scalar Foam::surfChemkinReader::molecularWeight
(
...
Foam::scalar Foam::surfChemkinReader::molecularWeightOf
(
    const word& specieName
) const
{
    speciesCompositionTable::const_iterator iter =
        speciesComposition_.find(specieName);

    if (iter == speciesComposition_.end())
    {
        FatalErrorInFunction
            << "Species " << specieName
            << " not found in speciesComposition table"
            << exit(FatalError);
    }

    return molecularWeight(iter());
}


void Foam::surfChemkinReader::checkCoeffs
(
...
void Foam::surfChemkinReader::addReactionType
(
...
void Foam::surfChemkinReader::addPressureDependentReaction
(
    const reactionType rType,
...
void Foam::surfChemkinReader::addReaction
(
    DynamicList<specieCoeffs>& lhs,
    DynamicList<specieCoeffs>& rhs,
    const scalarList& efficiencies,
...
void Foam::surfChemkinReader::readThermoOnly
(
...
)
{
    ...
    {
        FatalErrorInFunction
            << "Null thermo file provided for thermo-only mode"
            << exit(FatalError);
    }
...
void Foam::surfChemkinReader::read
(
...
Foam::surfChemkinReader::surfChemkinReader
(
