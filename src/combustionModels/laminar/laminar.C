...
void Foam::combustionModels::laminar<ReactionThermo>::correct()
{
    if (integrateReactionRate_)
    {
	    ...
	    Info << "deltaT is : " << this->mesh().time().deltaTValue() << endl;
        }
    }
    else
    {
...
{
    //Info << "  Nam --> check, in laminar class, calling R(Yi) func" << endl; //
...
// JaeHun
template<class ReactionThermo>
void Foam::combustionModels::laminar<ReactionThermo>::updateMassFractions()
{
        this->chemistryPtr_->updateMassFractions();
}

// Nam
template<class ReactionThermo>
//void Foam::combustionModels::laminar<ReactionThermo>::updateTfromTchem
void Foam::combustionModels::laminar<ReactionThermo>::updateYiHe
(
    PtrList<volScalarField>& Y,
    volScalarField& he
)
{
        //this->chemistryPtr_->updateTfromTchem(he);
        this->chemistryPtr_->updateYiHe(Y, he);
}


template<class ReactionThermo>
void Foam::combustionModels::laminar<ReactionThermo>::solveChemistry
(
    PtrList<volScalarField> Yold, 
    volScalarField p,
    volScalarField T,
    PtrList<volScalarField>& Ynew,
    volScalarField& Qdot
)
{
  /*
    // copy from correct() function 
    if (integrateReactionRate_)
    {
        if (fv::localEulerDdt::enabled(this->mesh()))
        {
            ...
            if (this->coeffs().found("maxIntegrationTime"))
            {
                ...
                this->chemistryPtr_->solveChemistry
                (
                    min(1.0/rDeltaT, maxIntegrationTime)(),
                    Yold, 
                    p,
...
template<class ReactionThermo>
void Foam::combustionModels::laminar<ReactionThermo>::setInitialFields
(
    PtrList<volScalarField> Yold,
    volScalarField p,
    volScalarField T
...
{
    this->chemistryPtr_->setInitialFields(Yold, p,T);
}


template<class ReactionThermo>
void Foam::combustionModels::laminar<ReactionThermo>::updateFieldsFromChem
(
    PtrList<volScalarField>& Ynew,
    volScalarField& Qdot
)
{
    this->chemistryPtr_->updateFieldsFromChem(Ynew,Qdot);
}
