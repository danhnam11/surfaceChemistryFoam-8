...
namespace Foam
{
    defineTypeNameAndDebug(basicCatThermo, 0);
    defineRunTimeSelectionTable(basicCatThermo, fvMesh);
}

const Foam::word Foam::basicCatThermo::dictName("catalystThermophysicalProperties");


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::wordList Foam::basicCatThermo::heBoundaryBaseTypes()
{
...
Foam::wordList Foam::basicCatThermo::heBoundaryTypes()
{
    const volScalarField::Boundary& tbf =
        this->T_.boundaryField();
...
Foam::volScalarField& Foam::basicCatThermo::lookupOrConstruct
(
    const fvMesh& mesh,
    const char* name
) const
...
Foam::basicCatThermo::basicCatThermo
(
    const fvMesh& mesh,
...
Foam::autoPtr<Foam::basicCatThermo> Foam::basicCatThermo::New
(
    const fvMesh& mesh,
    const word& phaseName
)
{
    return New<basicCatThermo>(mesh, phaseName);
}
...
const Foam::basicCatThermo& Foam::basicCatThermo::lookupThermo
(
    const fvPatchScalarField& pf
)
{
    if (pf.db().foundObject<basicCatThermo>(dictName))
    {
        return pf.db().lookupObject<basicCatThermo>(dictName);
    }
    else
    {
        HashTable<const basicCatThermo*> thermos =
            pf.db().lookupClass<basicCatThermo>();

        for
        (
            HashTable<const basicCatThermo*>::iterator iter = thermos.begin();
            iter != thermos.end();
            ++iter
        )
        {
...
void Foam::basicCatThermo::validate
(
    const string& app,
    const word& a
) const
...
Foam::wordList Foam::basicCatThermo::splitThermoName
(
...
const Foam::volScalarField& Foam::basicCatThermo::T() const
{
    return T_;
}


Foam::volScalarField& Foam::basicCatThermo::T()
{
    return T_;
}


const Foam::volScalarField& Foam::basicCatThermo::alpha() const
{
    return alpha_;
}


const Foam::scalarField& Foam::basicCatThermo::alpha(const label patchi) const
{
    return alpha_.boundaryField()[patchi];
}


bool Foam::basicCatThermo::read()
{
