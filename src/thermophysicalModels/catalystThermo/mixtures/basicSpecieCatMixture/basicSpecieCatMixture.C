...
#include "basicSpecieCatMixture.H"

// * * * * * * * * * * * * * Static Member Functions * * * * * * * * * * * * //

namespace Foam
{
    defineTypeNameAndDebug(basicSpecieCatMixture, 0);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::basicSpecieCatMixture::basicSpecieCatMixture
(
...
:
    basicCatMixture(thermoDict, mesh, phaseName),
    ...
    Y_(species_.size())
{
    ...
    forAll(species_, i)
    {
        ...
        else
        {
            ...
            if (!tYdefault.valid())
            {
                word YdefaultName(IOobject::groupName("Ysdefault", phaseName));
